...
![Image Laplacian Pyramid](/img/2017-11-26-papernotes_47/img_laplacian_pyramid.png)
...
![Sampling procedure of LAPGAN](/img/2017-11-26-papernotes_47/lapgan_sample_procedure.png)
...
![Train procedure of LAPGAN](/img/2017-11-26-papernotes_47/lapgan_train_procedure.png)
...
用 $f(x)$ 来表示 D 中间层的激活值, 那么 G 的新观测值就可以表示为 ![feature_matching_g_objective.png](/img/2017-11-26-papernotes_47/feature_matching_g_objective.png)
...
![mini-batch discrimination](/img/2017-11-26-papernotes_47/mini-batch_discrimination.png)
...
本文提出了一个评估样本的方法: 将每张生成的图片输入 Google Inception Net, 得到条件标签分布 $p(y\|x)$. 如果图中包含有意义的物体, 那么 $p(y\|x)$ 应具有一个低熵. 此外, 使用 GAN 都希望生成不同的图片 (样本), 即边缘 $\int p(y\|x=G(z))dz$ 应具有高熵. 因此, 结合两者, 得到了评估指标: ![inception_score_metric.png](/img/2017-11-26-papernotes_47/inception_score_metric.png), 对结果应用指数运算的目的是后续更容易比较. 该评估方法称为`Inception Score`
...
![semi-supervised_loss_function_decomposing.png](/img/2017-11-26-papernotes_47/semi-supervised_loss_function_decomposing.png)

此时 $L_{unsupervised}$ 实际上就是 GAN 的博弈值. 最小化 $L_{supervised}$ 和 $L_{unsupervised}$ 的最优解具有性质: $exp[l_j(x)]=c(x)p(y=i, x) \forall j<K+1$, $exp[l_{K+1}(x)]=c(x)p_G(x)$, 这意味着无监督损失和监督损失是一致的.

该半监督学习的方法引入了 G 与分类器之间的交互. 并且作者们发现使用`特征匹配`来优化 G 的效果特别好, 而 mini-batch discrimination 根本没起效.

最后, 作者们表示 K+1 的分类器是过参数化的, 因为从所有输出对率中减去 $f(x)$ 并不会改变 softmax 的输出. 这意味着可以等效地固定 $l_{K+1}(x)=0 \forall x$, 这样 $L_{supervised}$ 就变成了原来 K 类的分类器的标准监督损失函数, 那么 D 可以表示为 $D(x)=\frac{Z(x)}{Z(x)+1}$, $Z(x)=\Sigma_{k=1}^{K}exp[l_k(x)]$
